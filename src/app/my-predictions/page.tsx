...
import { Match, Prediction, Withdrawal } from "./types";
import HistoryTab from "./components/HistoryTab";
import NewPredictionsTab from "./components/NewPredictionsTab";
...
export default function MyPredictionsPage() {
  ...
  const handleCheckoutLocalCart = async () => {
    ...
    for (const [matchId, preds] of Object.entries(cart)) {
      ...
      for (const pred of preds) {
        ...
        if (seen.has(scoreKey)) {
          alert(
            `Gagal: Terdapat tebakan ganda ${pred.score_a} - ${pred.score_b} untuk laga ${matchName} di dalam keranjang belanja Anda.`
          );
          return;
        }
        seen.add(scoreKey);
      ...
      for (const pred of preds) {
        const isDuplicateDb = userPredictions.some(
          (dbPred) =>
        ...
        if (isDuplicateDb) {
          alert(
            `Gagal: Anda sudah pernah mengirim tebakan ${pred.score_a} - ${pred.score_b} untuk laga ${matchName} sebelumnya.`
          );
          return;
        }
      }
    ...
    try {
      ...
      if (paymentMethod === "cash") {
        alert(
          "Tebakan Anda berhasil diajukan! Status transaksi saat ini PENDING. Silakan lakukan pembayaran tunai ke meja panitia RT 12 sebesar Rp " +
  ...
  const payWithMidtrans = async (txId: string) => {
    try {
      ...
      if ((window as any).snap) {
        ...
        alert(
          "Gagal memuat sistem pembayaran Midtrans. Silakan coba beberapa saat lagi atau hubungi panitia."
        );
      }
    } catch (err: any) {
      alert("Gagal memproses transaksi: " + err.message);
  ...
  const handleRequestWithdrawal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Pengajuan penarikan dana sebesar Rp " +
          amountNum.toLocaleString("id-ID") +
          " berhasil dikirim! Silakan tunggu konfirmasi & transfer dari panitia."
      );
      setWdAmount("");
      await loadDashboardData(profile.id);
    } catch (err: any) {
      ...
      <div className="rounded-2xl bg-gradient-to-br from-card via-[#122218] to-card border border-border p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
        ...
        <div className="space-y-2 relative">
          ...
          <p className="text-xs text-muted-foreground">
            WhatsApp: <strong>+{profile.phone_number}</strong>
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-full md:w-auto relative">
          <div className="bg-background/80 border border-border/80 rounded-xl p-4 text-center space-y-1">
            ...
            <span className="text-lg font-black text-green-500">
              Rp {totalWinningsEarned.toLocaleString("id-ID")}
            </span>
          </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {activeSubTab === "history" && (
            <HistoryTab
              ...
              getMatchPoolStats={getMatchPoolStats}
          ...
          {activeSubTab === "new" && (
            <NewPredictionsTab
              ...
              cart={cart}
              addPredictionToCart={addPredictionToCart}
              getFlagUrl={getFlagUrl}
              ...
              updateCartScore={updateCartScore}
              removeCartPrediction={removeCartPrediction}
              handleCheckoutLocalCart={handleCheckoutLocalCart}
          ...
          {activeSubTab === "cart" && (
            <CartTab
              ...
              handlePayPendingTx={handlePayPendingTx}
            />
          )}
          ...
          {activeSubTab === "withdraw" && (
            <WithdrawTab
              ...
              setWdBank={setWdBank}
              wdNumber={wdNumber}
              setWdNumber={setWdNumber}
              wdName={wdName}
              setWdName={setWdName}
              wdSubmitting={wdSubmitting}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4">
          <div className="w-full max-w-xl bg-card border border-border rounded-2xl p-5 space-y-4 shadow-2xl relative">
            ...
            <div className="flex justify-center bg-background/50 rounded-xl overflow-hidden max-h-[70vh] p-2 border border-border/40">
              ...
              {previewReceiptUrl.startsWith("data:image") ||
              previewReceiptUrl.includes("http") ? (
                <img
                ...
                <div className="py-20 text-center text-xs text-muted-foreground">
                  ...
                  <p className="font-mono bg-muted p-3 mt-2 rounded font-bold break-all">
                    {previewReceiptUrl}
                  </p>
                </div>
              )}
