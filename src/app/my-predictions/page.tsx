                  ...
                  {filteredUpcomingMatches.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {filteredUpcomingMatches.map((match) => {
                        ...
                        return (
                          <div key={match.id} className="rounded-xl border border-border bg-card p-4 space-y-3 hover:border-primary/20 transition-all shadow-sm flex flex-col justify-between">
                            <div className="space-y-3">
                              ...
                              <div className="text-[10px] text-muted-foreground flex items-center justify-between border-t border-border/30 pt-2.5">
                                ...
                                <span className="text-[9px] bg-primary/10 text-primary px-2 py-0.5 rounded font-bold">
                                  {matchPaidPreds.length} Tebakan (Pool: Rp {(matchPaidPreds.length * 10000).toLocaleString("id-ID")})
                                </span>
                              </div>
                            </div>
                  ...
                  <div className="rounded-xl border border-border bg-card p-4 space-y-4 shadow-sm">
                    ...
                    {totalCartCount === 0 ? (
                      <div className="text-center py-10 text-[11px] text-muted-foreground border border-dashed border-border/40 rounded-lg space-y-1">
                        ...
                        <p className="text-[10px] text-muted-foreground/70">Tambahkan tebakan dari daftar laga di samping kiri.</p>
                      </div>
                    ) : (
                      <>
                        <div className="space-y-3 max-h-[320px] overflow-y-auto pr-1">
                          {Object.entries(cart).map(([matchId, preds]) => {
                            ...
                            if (!match) return null;
                            return (
                              <div key={matchId} className="border border-border/20 bg-background/40 rounded-lg p-2.5 space-y-2 text-xs">
                                <div className="font-bold text-[10px] truncate text-foreground">{match.team_a} vs {match.team_b}</div>
                                <div className="space-y-1.5">
                                  {preds.map((pred, idx) => (
                                    <div key={idx} className="flex items-center justify-between bg-card/60 border border-border/10 p-1.5 rounded text-[10px] gap-2">
                                      ...
                                      <div className="flex items-center space-x-1 justify-end flex-1">
                                        <input
                                          ...
                                          className="w-8 h-6 text-center rounded border border-input bg-background font-bold text-[10px]"
                                        />
                        ...
                        <div className="border-t border-border/40 pt-3.5 space-y-3">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <span className="text-sm font-black text-primary">Rp {totalCartPrice.toLocaleString("id-ID")}</span>
                          </div>

                          <div className="space-y-1.5">
                            <label className="text-[9px] text-muted-foreground uppercase font-bold tracking-wider block">Metode Pembayaran</label>
                            <select
                              value={paymentMethod}
